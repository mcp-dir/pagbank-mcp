...
### PagBank no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **PagBank** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do PagBank em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **PagBank via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `PagBank MCP` e **URL** `https://api.mcp.ai/p_pagbank`.

### Cursor

[➕ Instalar PagBank MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=pagbank&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9wYWdiYW5rIn0=)

### VS Code (Copilot Chat)

[➕ Instalar PagBank MCP no VS Code](vscode:mcp/install?name=pagbank&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_pagbank%22%7D)
...
```
Quanto gastei no cartão do PagBank esse mês?
Resuma minha fatura do PagBank em aberto
Qual meu saldo na conta do PagBank?
Como está minha carteira de investimentos no PagBank?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta PagBank com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o PagBank?**
Este MCP é focado no PagBank. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [pagbank@mcp.ai](mailto:pagbank@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/pagbank-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_pagbank` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
